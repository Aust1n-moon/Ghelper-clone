...
echo ""
echo "[2/6] Checking GNOME AppIndicator extension..."
...
echo ""
echo "[3/6] Installing application launcher..."
...
echo ""
echo "[4/6] Setting up autostart on login..."
...
echo ""
echo "[5/6] Installing sleep/suspend power hook..."
...
# 6. Disable d3cold for MT7922 WiFi (prevents firmware reinit failures after suspend)
echo ""
echo "[6/6] Installing WiFi suspend fix (MT7922 d3cold disable)..."
UDEV_RULE='ACTION=="add", SUBSYSTEM=="pci", ATTR{vendor}=="0x14c3", ATTR{device}=="0x0616", ATTR{d3cold_allowed}="0"'
echo "$UDEV_RULE" | sudo tee /etc/udev/rules.d/99-mt7922-no-d3cold.rules > /dev/null
